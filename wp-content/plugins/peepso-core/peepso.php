...
class PeepSo
{
    ...
    private function __construct()
    {
        ...
        if (is_admin() && (!defined('DOING_AJAX') || !DOING_AJAX)) {
            ...
            // FSTVL
            if(0 == PeepSo::get_option('override_fstvl', 0)) {
                add_filter('pre_set_site_transient_update_plugins', function ($value, $expiration = NULL, $transient = NULL) use ($path) {
                    ...
                    if (isset($value->response) && isset($value->response[$path])) {
                ...
                add_action('after_plugin_row_' . $path, function ($plugin_file, $plugin_data = NULL, $status = NULL) use ($path) {
                    ...
                    // debug
                    #$new_version = "2.0";
                    #$conflict=array('Debug Plugin'=>"1.0");
                        ...
                        <tr class="plugin-update-tr active">
                            <td colspan="3">
                                <div class="update-message notice inline notice-warning notice-alt">
                                    <p>
                                        <strong>
                                            <?php echo sprintf(__('There is a new version of PeepSo available (%s), but it cannot be installed yet.', 'peepso-core'), $new_version); ?>
                                        </strong>
                                    </p>
                                    <br/>
                                ...
                                <strong>
                                    ...
                                    <strong><?php echo __('After updating these plugins please refresh this page, update PeepSo and reactivate all plugins.'); ?></strong>

                                    <br/>
                                    <p><?php echo sprintf(
                                            __('If you are experiencing issues updating any plugins, try using the <a href="%s">Updates Dashboard</a>.','peepso-core'),
    ...
    public function peepso_save_post_action($post_id, $post, $update)
    {
        ...
        if ($option != $content && $post->post_type == 'page') {
            ...
            $page_url = str_replace(site_url(), '', get_permalink($post_id));
            $page_url = trim(rtrim($page_url, '/'), '/');
            $settings->set_option($option, $page_url);
        }
    }
...
// todo: handling oEmbed when visiting peepso Page
    ...
    public function remove_footer_admin()
    {
        ...
        if(substr($page, 0,6) == 'peepso') {
            echo '<p id="footer-left" class="alignleft">If you like <strong>PeepSo</strong> please leave us a <a href="https://wordpress.org/support/view/plugin-reviews/peepso-core?filter=5#postform" target="_blank" class="wc-rating-link" data-rated="Thanks :)">★★★★★</a> rating. Thank you in advance! And have a great time using PeepSo! || <span class="psa-social-text">Find and Follow PeepSo on Social Media:</span> <a class="psa-social-link" href="https://PeepSo.com/community/" target="_blank"><img src="https://peepso.com/newsletter/main_elements/social_media/peepso_community_icon.png" alt="PeepSo Community"></a> <a class="psa-social-link" href="https://facebook.com/peepso/" target="_blank"><img src="https://peepso.com/newsletter/main_elements/social_media/facebook.png" alt="PeepSo Facebook"></a> <a class="psa-social-link" href="https://instagram.com/peepsowp/" target="_blank"><img src="https://peepso.com/newsletter/main_elements/social_media/instagram.png" alt="PeepSo Instagram"></a> <a class="psa-social-link" href="https://twitter.com/peepsowp/" target="_blank"><img src="https://peepso.com/newsletter/main_elements/social_media/twitter.png" alt="PeepSo Twitter"></a></p>';
        }
    }
    ...
    public function init_callback()
    {
        ...
        if(class_exists('PeepSoGroupsPlugin') || class_exists('PeepSoFriends')) {
            add_filter('peepso_stream_id_list', function ($filters) {
                ...
                if(class_exists('PeepSoFriendsPlugin'))    {   $who[] = __('members', 'peepso-core');   }
                if(class_exists('PeepSoGroupsPlugin'))     {   $who[] = __('groups', 'peepso-core');    }
                if(class_exists('PeepSoPagesPlugin'))      {   $who[] = __('pages', 'peepso-core');     }
...
// @todo HTML rendering methods should probably be refactored
    ...
    public static function license_notice($plugin_name, $plugin_slug, $forced=FALSE)
    {
        ...
        switch ($license_data['response']) {
            ...
            case 'item_name_mismatch':
            default:
                $message = __('License for PLUGIN_NAME is missing or invalid. Please <a href="ENTER_LICENSE">enter a valid license</a> and click "SAVE" to activate it. You can get your license key <a target="_blank" href="PEEPSOCOM_LICENSES">here</a>.', 'peepso-core');
                break;
        }
    ...
    public static function maintenance_notice()
    {
        ...
        echo '<div class="error peepso"><strong>' .__('It looks like PeepSo Maintenance scripts were not processing properly. We just tried to fix it automatically.<br> If you see this message repeatedly, consider deactivating and re-activating PeepSo or contacting Support', 'peepso-core').'</strong></div>';
    }
    ...
    public static function gdpr_external_cron_notice()
    {
        ...
        echo '<div class="error peepso"><strong>' .__('It looks like PeepSo GDPR compliance were not processing properly. We just tried to fix it automatically.<br> If you see this message repeatedly, consider deactivating and re-activating PeepSo or contacting Support', 'peepso-core').'</strong></div>';
    }
    ...
    public static function version_notice($plugin_name, $plugin_slug, $version_check, $legacy = TRUE)
    {
        ...
        <div class="error peepso" style="<?php echo(-1==$version_check['compat']) ? 'border-left-color: orange;':'';?>">
            ...
            if($version_lock) {
                // Please upgrade
                printf(__('Please upgrade %s and PeepSo to avoid conflicts and issues. ', 'peepso-core'), $plugin_name);
