(function( $, factory ) {
	...
	function PsNotification() {
		...
		ps_observer.add_action( 'notification_start', start );
	}
	...
	PsNotification.prototype = {
		...
		markAsRead: function( id ) {
			return $.Deferred( $.proxy( function( defer ) {
				...
				peepso.postJson( 'notificationsajax.mark_as_read', params, $.proxy( function( json ) {
					if ( json.success ) {
	...
	function PsPopoverNotification(elem, options)
	{
		...
		this.init = function(opts) {
			...
			if ( footerLinks ) {
				...
				this.popover_footer = $( '<div class="ps-popover-footer app-box-footer ps-clearfix"></div>' );
			...
			this.popover_list.on( 'mousedown.ps-notification', '.ps-js-notification a', $.proxy( function( e ) {
				...
				peepso.notification.markAsRead( $item.data( 'id' ) ).done( $.proxy( function() {
					...
					$( '.ps-js-notifications' ).find( '.ps-js-counter' )
						.html( count - 1 ).css( 'display', count > 1 ? '' : 'none' );
			...
			this.popover_list.on( 'mousedown click', '.ps-js-mark-as-read', $.proxy( function( e ) {
				var $icon, $item;
			...
			this.popover_footer.on( 'click', '.ps-js-mark-all-as-read', $.proxy( function( e ) {
				...
				if ( confirm( peepsodata.mark_all_as_read_confirm_text ) ) {
					...
					peepso.notification.markAllAsRead().done(function() {
						$icons.remove();
						$items.css( 'opacity', '' );
						$items.data( 'unread', 0 );
			...
			// Toggle unread only.
			this.popover_footer.on( 'click', '.ps-js-toggle-unread-only', $.proxy( function( e ) {
				var $btn = $( e.currentTarget );

				this._unreadOnly = ! this._unreadOnly;
				$btn.html( this._unreadOnly ? peepsodata.show_all_text : peepsodata.show_unread_only_text );
