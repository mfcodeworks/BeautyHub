...
export default class NotificationPopoverGeneral extends NotificationPopover {
    ...
    constructor( elem ) {
        ...
        instance.on( 'counter:updated', count => { this.updateCounter( count ) });
        instance.on( 'html:updated', html => { this.updateHtml( html ) });
        instance.on( 'unread_only:updated', state => { this.updateButton( state ) } );
    }
    ...
    createBody() {
        return super.createBody()
            .on( 'mousedown', '.ps-js-notification', ( e ) => { this.onItemClick( e ) })
            .on( 'click', '.ps-js-mark-as-read', ( e ) => { this.onMarkRead( e ) })
            // Do not propagate these events which might triggers unwanted ancestor's event handlers.
            .on( 'click', '.ps-js-notification a', ( e ) => { e.stopPropagation() })
            .on( 'mousedown', '.ps-js-mark-as-read', ( e ) => { e.stopPropagation() });
    }
    ...
    createFooter() {
        ...
        this.$btnMarkAllRead = $( '<a href="#">' + TEXT_MARK_AS_READ + '</a>' )
            .css({ float: 'left', width: '50%' })
            .on( 'click', ( e ) => { this.onMarkAllRead( e ) });

        // Toggle unread only button.
        this.$btnToggleUnreadOnly = $( '<a href="#">' + TEXT_SHOW_UNREAD_ONLY + '</a>' )
            .css({ float: 'left', width: '50%' })
            .on( 'click', ( e ) => { this.onToggleUnreadOnly( e ) });

        return $( '<div class="ps-popover-footer app-box-footer ps-clearfix"></div>' )
    ...
    onItemClick( e ) {
        ...
        if (
            ! isUnread // Item is alread read, just do default action to follow link URL.
    ...
    onMarkAllRead( e ) {
        ...
        if ( confirm( TEXT_MARK_AS_READ_CONFIRM ) ) {
            $btn.data( 'progress', true ).css( 'opacity', .5 );
            instance.markRead().then(() => {
                this.toggle();
            }).catch( $.noop ).then(() => {
    ...
    onToggleUnreadOnly( e ) {
        ...
        this.$popoverBody.empty()
            .append( this.$popoverLoading );

        $btn.data( 'progress', true ).css( 'opacity', .5 );
        instance.toggleUnreadOnly().catch( $.noop ).then(() => {
            $btn.data( 'progress', false ).css( 'opacity', '' );
            this.tryLoadNext();
        });
    }
