...
PsMember.prototype.block_user = function( userId, elem ) {
	...
	func[ blockId ] = true;
	peepso.postJson( 'activity.blockuser', params, $.proxy( function( json ) {
...
PsMember.prototype.unblock_user = function( userId, elem ) {
	...
	func[ unblockId ] = true;
	peepso.postJson( 'activity.unblockuser', params, $.proxy( function( json ) {
		var $focus = $( '.ps-js-focus--' + userId ),
			$actions = $focus.find( '.ps-focus-actions, .ps-focus-actions-mobile' ),
			data = json.data;
		...
		if ( json.success ) {
			...
			// Remove item.
			$( elem ).closest( '.ps-members-item-wrapper' ).fadeOut( 'fast' );
		}
	}, this ) );
};
