...
ps_login.form_submit = function(e)
{
	...
	// Send login request.
	ajax = peepso.postJson( 'auth.login', data, jQuery.noop ).ret;
	ajax.always(function( data /* or jqXHR */ ) {
		...
		if (response.success) {
			...
			try {
				var title = response.data && response.data.dialog_title || '';
				pswindow.hide();
				if ( false === pswindow.acknowledge(response.errors, title) ) {
					form.find(".errlogin").html(response.errors[0]).css("display", "block");
				}
				var codes = response.data && response.data.error_code || [];
				if ( codes.indexOf( 'pending_approval' ) > -1 ) {
					jQuery( '.ps-js-register-activation' ).show();
				}
			} catch( e ){}
		} else { // Assume non-JSON response.
...
jQuery(function( $ ) {
	...
	if ( ! ( +peepsodata.external_link_warning ) ) {
		return;
	}

	// Replaces external links found in an element.
	function replaceLinks( $elem, siteUrl, gatewayUrl ) {
		// Treats http and https the same way. Also, "www" is optional.
		siteUrl = siteUrl
			.replace( /^https?:\/\/(www\.)?/i, '' )
			.replace( /\/+$/, '' );

		// Check and replace every external links.
		$elem.find( 'a' ).each(function() {
			var $link = $( this ),
				href = $link.attr( 'href' );

			if ( href && href.match( /^https?:\/\//i ) ) {
				if ( ! href.match( new RegExp( '^https?://(www\\.)?' + siteUrl ) ) ) {
					if ( ! ( +$link.data( 'noHijack' ) ) ) {
						$link.attr( 'href', gatewayUrl + '?url=' + href );
					}
				}
			}
		});
	...
	function hijackLinks() {
		replaceLinks(
			$( '#peepso-wrap, .ps-widget--external, .ps-lightbox-wrapper' ),
			peepsodata.site_url,
			peepsodata.external_link_warning_page
		);
	}
	...
	peepso.observer.addAction( 'peepso_external_link', function( $elem ) {
		replaceLinks(
			$elem,
			peepsodata.site_url,
			peepsodata.external_link_warning_page
