...
class PeepSoGdpr
{
	...
	public static function process_export_data()
	{
		...
		foreach ($queue as $request) {
			...
			$success = self::do_export_data($request->request_user_id);
			...
			if (FALSE === $success) {
				...
				$request_file_path = $user->get_personaldata_dir() . $user->get_username() . '.zip';
				$request_file_url = $user->get_personaldata_url() . $user->get_username() . '.zip';
	 ...
	 */
	public static function do_export_data($user_id)
	{
		$user = PeepSoUser::get_instance($user_id);
		...
		self::generate_zip_file($user);

		return TRUE;
	}

	public static function generate_zip_file($user) {
		...
		$zip = new ZipArchive();
		$zip->open($user->get_personaldata_dir() . DIRECTORY_SEPARATOR . $user->get_username() . '.zip', ZipArchive::CREATE | ZipArchive::OVERWRITE);
	...
	public static function export_videos($user, $type='')
	{
        ...
        $list_videos = $videos_model->get_user_videos($user->get_id(), 0, 999999999, 'desc', 0);
