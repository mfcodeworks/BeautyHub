	...
	public function pre_user_query(WP_User_Query $wp_user_query)
	{
		...
		if( isset($wp_user_query->query_vars['orderby']) && isset($wp_user_query->query_vars['order']) ) {
			...
			if(stristr($order_by, 'peepso_')) {
				...
				$wp_user_query->query_from .= ' INNER JOIN (SELECT * FROM `' . $wpdb->usermeta . '` WHERE `meta_key` = \'first_name\' GROUP BY `user_id`) `psmeta1`
					ON `' . $wpdb->users . '`.`ID` = `psmeta1`.`user_id` ';
				$wp_user_query->query_from .= ' INNER JOIN (SELECT * FROM `' . $wpdb->usermeta . '` WHERE `meta_key` = \'last_name\' GROUP BY `user_id`) `psmeta2`
