...
//              $notice .= ' <a href="#TB_inline?inlineId=assign-roles-modal-id" class="thickbox">' . __('Click here', 'peepso-core') . '</a>' . __(' for more information on assigning roles.', 'peepso-core');
        ...
        if (NULL === $this->dashboard_tabs) {
            ...
            $tabs = array(
				'blue' => array(
					...
					'requests' => array(
						'slug' => 'peepso-gdpr-request-data', // peepso-messages',
						'menu' => __('Request Data (BETA)', 'peepso-core'),
						// 'icon' => 'download', // 'envelope',               // dashicons-email
			...
			foreach ($report_items as $item)
			{
				...
				switch ($item['rep_module_id'])
				{
					case 0 :
						...
						break;
					default :
						echo '<a class="psa-list--reported__link" href="' . PeepSo::get_page('activity_status')  . $item['post_title'] . '/" target="_blank">' . $item['post_title'] . ' <i class="fa fa-external-link"></i></a>';
						echo '<a class="btn btn-xs" href="' . admin_url('admin.php?page=peepso&action=update-report&set=dismiss&id=' . $item['rep_id'] . '&_wpnonce=' . $nonce) . '">Dismiss</a>';
	...
	public function get_plugin_list() {
		...
		if (empty($check_plugins)) {
			$response = wp_remote_get(self::PEEPSO_URL . '/products.json', array('timeout' => 15));
