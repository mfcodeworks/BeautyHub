...
class PeepSoMoods {
	...
	public function enqueue_scripts()
	{
		...
		if ($this->is_enabled) {
			wp_enqueue_script('peepso-moods', PeepSo::get_asset('js/moods/bundle.min.js'), array('peepso', 'peepso-postbox'), PeepSo::PLUGIN_VERSION, TRUE);
		}
	}
	...
	public function filter_postbox_interactions($out_html = array())
	{
		...
		{
			$mood_list .= "
				<a class='mood-list' id='postbox-mood-{$id}' href='javascript:' data-option-value='{$id}' data-option-display-value='{$mood}'>
		...
		$mood_data = array(
			...
			'icon' => 'happy',
			'click' => 'return;',
			'title' => __('Mood settings for your post', 'peepso-core'),
			'extra' => "<div id='postbox-mood' class='ps-dropdown__menu ps-dropdown__menu--moods ps-js-postbox-mood' style='display:none'>
							{$mood_list}
							<button id='postbox-mood-remove' class='ps-btn ps-btn-danger ps-remove-location' style='width:100%; display:none'><i class='ps-icon-remove'></i>{$mood_remove}</button>
						</div>{$mood_ux}"
		);
