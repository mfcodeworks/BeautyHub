	...
	private function _render_access($name, $field)
	{
		if (isset($field['access']) && -1 !== intval($field['access'])) {
			...
			foreach ($aAccess as $nAcc => $aAcc) {
				echo '<a id="', $name, '-acc-', $nAcc, '" href="javascript:" data-option-value="', $nAcc, '">';
	...
	private function _render_field_type($name, $field)
	{
		...
		switch ($field['type'])
		{
			...
			case 'access-profile-post':
				...
				foreach ($aAccess as $nAcc => $aAcc) {
					$sField .= '<a id="' . $name . '-acc-' . $nAcc . '" href="javascript:" data-option-value="' . $nAcc . '">';
		...
		if (!empty($field['descript']))
			$sField .= '<p class="ps-form__label-desc lbl-descript">'.$field['descript'].'</p>';
