...
//				PeepSoMailQueue::add($owner_id, $data, __('Someone liked your post', 'peepso-core'), 'likepost');
    ...
    public function blockuser(PeepSoAjaxResponse $resp)
    {
        ...
        if (PeepSo::check_permissions($user_id, PeepSo::PERM_POST, $user_id)) {
            ...
            $resp->success(1);
            $resp->set('redirect', PeepSo::get_page('members').'blocked/');
        }
    }
...
//        }
    ...
    public function post_options()
    {
        ...
        echo	'<a href="javascript:" class="ps-dropdown__toggle ps-js-dropdown-toggle" data-value="">', PHP_EOL;
    ...
    public function show_posts_per_page(PeepSoAjaxResponse $resp)
    {
        ...
        // If loading pinned posts along with the other posts (not single post view)
        if(1==$this->_input->int('pinned', 0) && 0==$post_id) {
    ...
    public function post_action_title()
    {
        ...
        if ($post->post_author === $post->act_owner_id) {
            ...
            $title = sprintf('<a class="ps-stream-user" href="%s">%s</a> <span class="ps-stream-action-title">%s</span> ',
                $user->get_profileurl(), $before_fullname . $user->get_fullname() . $after_fullname, $action);
        } else {
            $author = PeepSoUser::get_instance($post->post_author);
